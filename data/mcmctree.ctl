       ...
       seqfile = ../concat.phy		* Path to input sequence file (e.g., many alignments in a unique file)
      treefile = ../mcmctree.phy	* Path to input tree file including calibration points as node labels	
       outfile = out.log		* Path to log output file
      mcmcfile = mcmc.txt		* Path to output file with MCMC samples

         ndata = 35			* Number of partitions
  ...
  sigma2_gamma = 1 10 1			* gamma prior for sigma^2 (for clock=2 or 3)
    ...
    checkpoint = 1			* turn checkpointing on with 1, change to 2 if resume is required after a failed one where 1 was active
        burnin = 5000			* Number of iterations that will be discarded as part of burn-in
      sampfreq = 50			* Sampling frequency
       nsample = 10000			* Number of samples to be collected
