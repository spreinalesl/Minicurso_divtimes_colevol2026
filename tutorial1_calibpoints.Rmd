...
Dado que la topología que vamos a fijar para el análisis con `MCMCtree` no debe tener longitudes de rama:
...
```{bash eval=FALSE, class.source='klippy'}
# leer los datos
dataset = CSV.read("data/Istiophoridae.tsv", delim = '\t', DataFrame)

# calcular el punto medio del intervalo de edad de cada fosil para una edad puntual
data = (dataset[:,6] .+ dataset[:,5])./2

# definir priors
theta1_prior = RefOffExponential(1, maximum(data), 1) # RefOffExp partiendo de la ocurrencia más antiga
theta2_prior = 0.0 # theta2 fijado en el presente, el grupo es viviente
lambda_prior = Normal(0.0, 4.0) # un prior amplio para lambda que debería ser informado por los datos 

# especificar el objeto stratinterval
istio_interval = StratInterval(data, theta1_prior, theta2_prior, lambda_prior)

# muestrear por MCMC
post_sample = sample_stratinterval(istio_interval, 10000, NUTS(), false, false)

# graficar las cadenas de Markov
plot(post_sample)

# graficar todo: datos, priors, posteriores
histogram(data, bins=range(minimum(data), stop = maximum(data), length = 18), normalize=true, label="Data")
plot!(theta1_prior, label="Prior", linewidth=3)
density!(post_sample[:,"θ1",:], label="Posterior", linewidth=3)

# examinar las estadísticas sumario de las distribuciones posteriores
julia> describe(post_sample)[1]
Summary Statistics
  parameters      mean       std      mcse    ess_bulk    ess_tail      rhat   ess_per_sec 
      Symbol   Float64   Float64   Float64     Float64     Float64   Float64       Float64 

          θ1   58.6608    0.9301    0.0131   4177.0745   3355.4383    1.0000      331.1459
           λ   -0.3006    0.1916    0.0030   4351.3636   3344.6220    1.0000      344.9630


julia> describe(post_sample)[2]
Quantiles
  parameters      2.5%     25.0%     50.0%     75.0%     97.5% 
      Symbol   Float64   Float64   Float64   Float64   Float64 

          θ1   57.6559   57.9813   58.4018   59.0511   61.1161
           λ   -0.7177   -0.4243   -0.2857   -0.1601    0.0194
```

Viendo el gráfico de densidad del tiempo de originación, la moda de la curva está casi en 58.0. Usamos entonces tL siendo el cuantil 2.5%, p = moda - cuantil\_2.5% = 0.4 Ma, es decir 0.004 en unidades de 100Ma, los cuantiles 2.5% y 97.5% de la función describe arriba, y la función c_truncauchy para encontrar c:

```{r, class.source='klippy'}
## Vamos a estimar el parametro c de la distribucion L
cistio <- c_truncauchy(tl=0.5765, tr=0.611,
                       p=0.0035, pr=0.975, al=0.025)

print(cistio)
0.003424076
```

El prior de calibración usando este método fue definido de la siguiente forma: `L(0.5765,0.0035,0.00342,0.025)`

```{r, class.source='klippy'}
calib$calibstring[which(calib$id == "n56")] <- "'L[0.5765@0.0035@0.00342@0.025]'"
```
