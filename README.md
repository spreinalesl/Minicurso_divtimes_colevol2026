...
En este tutorial vamos a estimar los tiempos de divergencia para la familia XXXXX usando calibración de nodos. A continuación listamos los elementos necesarios para correr este tipo de análisis en ```MCMCtree```:
...
- Establecer los *priors* de calibración [Tutorial1](). Para esto necesitamos definir cuáles nodos van a ser calibrados, escoger la distribución estadística (y su respectivo conjunto de parámetros) que vamos a usar para informar el tiempo en cada uno de los nodos, y finalmente incluir esa información en el archivo del árbol.   
